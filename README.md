...
- `train.py` — 전체 파이프라인 오케스트레이션. 3D-CNN 학습(LR=0.001) → 3D-ResNet
  ...
  (논문 Table 5). `--smoke_test` 플래그로 데이터 수/epoch/WOA 반복만 축소한 경량 검증
  실행도 지원합니다(모델 구조 자체는 축소하지 않음).
...
논문에 명시되지 않아 프로젝트가 자체적으로 결정한 항목은 모두
`03_Model_Training/DEVIATIONS.md`에 근거와 함께 기록되어 있습니다. 주요 항목만
요약합니다.

| 구성 요소 | 항목 | 논문 명시 여부 | 채택 값 | 근거 |
|---|---|---|---|---|
| 3D-CNN | Conv Stride/Padding | 미명시 | stride=1, padding=1 (SAME) | Figure 2의 단계별 출력 크기가 pool 전까지 불변으로 표기됨 |
| 3D-CNN | Flatten 차원 | 명시(12544)하나 산술 불일치(7×7×7×256=87808) | AdaptiveAvgPool로 depth축만 축소 → 12544 | 논문 명시 수치를 재현 목표로 우선시 |
| 3D-CNN | FV-3 결합 방식 | Eq.2는 elementwise max(→1000)나 Fig.1 라벨은 ×2000 | concatenation(2000차원) | 최종 차원이 Fig.1과 일치하도록 채택 |
| 3D-ResNet | 채널 폭 | 전혀 미명시 | 64→128→256→512→512 | 표준 ResNet 채널 더블링 관행 적용 |
| CCA | 성분 개수 | 미명시 | `min(n_samples-1, 100)` | 샘플 수·특징 차원에 안전하게 맞춤 |
| WOA | fitness 가중치 alpha | 미명시 | 0.99 | WOA 특징선택 문헌의 통상값 |
| 분류기 | SVM/KNN/GB/RF 하이퍼파라미터 | 전혀 미명시 | scikit-learn 기본값 | 논문에 근거 없음 |
| 학습 | 배치 크기 | 미명시(그리드 후보 32/64) | 32 | 메모리 안전한 값 채택 |
| 학습 | Grid Search 자체 재실행 | 명시되나 재탐색 미수행 | 논문이 보고한 최종 채택값(Adam, LR 0.001/0.0001) 직접 사용 | 그리드서치 전체 재실행은 계산비용이 지나치게 큼 |
