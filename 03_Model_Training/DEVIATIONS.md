...
| Batch size | **명시됨** (Table 3, Study 2, Variant 3 — 3D-CNN 최고 성능 구성, Accuracy 93.41%가 Batch=64 사용) | 유효 배치 64 (물리 배치 8 × gradient accumulation 8회) | 이 아키텍처는 56³ 입력에서 채널을 최대 1024까지 사용해 배치 64를 물리적으로 그대로 올리면 활성화 메모리가 24GB 이상 필요할 수 있음(2026-07-03 재추정). 12GB급 GPU에서도 안전하게 논문과 수치적으로 동일한 유효 배치를 재현하기 위해 gradient accumulation 채택. *(2026-07-03 정정: 이전에는 "논문에 배치 크기가 없다"고 기록했으나, Table 3을 재확인한 결과 실제로는 명시되어 있었음을 확인함)* |
