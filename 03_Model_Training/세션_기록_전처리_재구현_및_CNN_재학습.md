...
새 스크립트 `01_Preprocessing/스크립트/preparing_ref21order.py`로 기존 `preparing.py`와
...
- 출력: `01_Preprocessing/전처리_ref21order/`
...
6. **`--csv_path`/`--image_dir` 기본값**이 stale했던 문제(존재하지 않는
   ...
   2026-07-20, 여러 전처리 변형을 비교해 `전처리_ref21order`(min-max, N4 없음)를
   표준으로 채택하기로 하면서 기본값을 `data_0713_wsl_v2.csv` +
   `전처리_ref21order`로 최종 변경(이제 `--image_dir` 안 줘도 이 경로 사용).
